...
module.exports = class Frames extends Collection
  ...
  fetch_filtered:(options) =>
    params = {
      ...
      limit: 20
      skip: 0
      sort: JSON.stringify({'capturetime_epoch': -1 })
    }
    if options && options.filter
      if options.filter.time_from || options.filter.time_to
        options.filter.capturetime_epoch = {}
        if options.filter.time_from
          options.filter.capturetime_epoch['$gte'] = {'$date': options.filter.time_from}
          delete options.filter.time_from
        if options.filter.time_to
          options.filter.capturetime_epoch['$lte'] = {'$date': options.filter.time_to}
    ...
    if options && options.add
      add = options.add
    fetch_params = {add: add, data: params}
